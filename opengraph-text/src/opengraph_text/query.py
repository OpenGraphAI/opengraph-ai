"""Query the knowledge graph and answer questions over extracted document data.
...
_NODE_TYPES = ["document", "entity", "topic", "attribute", "claim"]
...
def _resolve_node_id(graph: DocumentGraph, ref: str) -> str:
...
def find_nodes_by_label(graph: DocumentGraph, pattern: str, node_type: str | None = None) -> list[dict]:
...
def find_nodes_by_type(graph: DocumentGraph, node_type: str) -> list[dict]:
...
def get_neighbors(graph: DocumentGraph, node_id: str, depth: int = 1) -> dict:
...
def find_documents_mentioning(graph: DocumentGraph, entity_labels: list[str]) -> list[str]:
    """Return document IDs whose contained entities fuzzy-match ALL of entity_labels."""
    document_ids = []
    for node_id, attrs in graph._g.nodes(data=True):
...
def filter_documents_by_topic(graph: DocumentGraph, topic_label: str) -> list[str]:
    """Return document IDs that are about a topic fuzzy-matching topic_label."""
    label_lower = topic_label.lower()
    matching_topic_nodes = [
        node_id
        for node_id, attrs in graph._g.nodes(data=True)
        if attrs.get("type") == "topic" and label_lower in attrs.get("label", "").lower()
    ]

    document_ids: set[str] = set()
    for topic_node in matching_topic_nodes:
        for src, _, edata in graph._g.in_edges(topic_node, data=True):
            if edata.get("relation") == "about_topic" and graph._g.nodes[src].get("type") == "document":
                document_ids.add(src)
...
_TOOLS = [
    ...
    },
    {
        ...
        "description": "Return all nodes of a given type (document, entity, topic, attribute, claim).",
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": "Return document IDs whose contained entities fuzzy-match ALL of the given labels.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "Return document IDs that are about a topic fuzzy-matching the given topic label.",
        "input_schema": {
            "type": "object",
            "properties": {
...
_SYSTEM_PROMPT = (
    "You are answering questions about a text knowledge graph. Use the provided tools "
    "to find relevant nodes and subgraphs. Then write a natural-language summary citing "
    "document filenames and entity labels. Be specific."
)


def _build_context(graph: DocumentGraph) -> str:
    """Build a compact (<2k token) summary of the graph for the model's first message."""
    s = graph.summary()
    entity_labels = sorted(
        {
            attrs.get("label")
            for _, attrs in graph._g.nodes(data=True)
            if attrs.get("type") == "entity" and attrs.get("label")
        }
    )
    ...
    lines = [
        ...
        f"Documents ({len(s['documents'])}): {', '.join(s['documents'])}",
        f"Sample entity labels: {', '.join(entity_labels[:40])}",
    ]
    return "\n".join(lines)
...
def _build_subgraph(graph: DocumentGraph, node_ids: set[str]) -> dict:
...
def _execute_tool(graph: DocumentGraph, name: str, tool_input: dict) -> Any:
